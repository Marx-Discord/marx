...
void main() async {
  ...
  await dir.list(recursive: false, followLinks: false)
  .listen((FileSystemEntity entity) async {
    var fileName = entity.path.split('/').last.split('\\').last.replaceFirst('.sql', '');
    ...
    for(final row in response) {
      ...
      if(!migrated_once) {
        await database.connection.query('CREATE TABLE IF NOT EXISTS migrations (migration TEXT UNIQUE)');
      } else {
        var migratedResponse = await database.connection.query('SELECT true FROM migrations WHERE migration = @migration', substitutionValues: {'migration': fileName});
    ...
    if(contents != null) {
      ...
      await database.connection.query('INSERT INTO migrations (migration) VALUES (@migration)', substitutionValues: { 'migration': fileName });
    }
  });
}